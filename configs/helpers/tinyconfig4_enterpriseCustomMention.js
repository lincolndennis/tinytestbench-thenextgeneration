...
if ((location.search) && (location.search.indexOf("?spellchecker_dialog") != -1)) {
    var params = location.search;
    dialogFlag = params.replace("?spellchecker_dialog=", "");
}
...
var mentions = {
    ...
    initParams: {
        ...
        mentions_selector: 'user.personDetails',
        mentions_menu_complete: function (editor, userinfo) {
            var customTag = editor.getDoc().createElement('user');
            customTag.className = 'personDetails';
            customTag.appendChild(editor.getDoc().createTextNode('@' + userinfo.name));
            return customTag;
        },
